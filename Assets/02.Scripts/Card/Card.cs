...
public class Card : MonoBehaviour
{
    ...
    // 카드의 이름, 내용 소개
    void CardNameAndDetail(int num)
    {
        switch (num)
        {
            case 0:
                if (ShotGunCard == 2) { CardName = "샷건개조 Level 3"; CardDetail = "기존 투사체에서 +4"; }
                else if (ShotGunCard == 1) { CardName = "샷건개조 Level 2"; CardDetail = "기존 투사체에서 +2"; }
                else { CardName = "샷건개조 Level 1"; CardDetail = "기존 투사체에서 +1"; }
                break;
            case 1:
                if (BulletCard == 1) { CardName = "총알개조 Level 2"; CardDetail = "적 두 명 관통"; }
                else { CardName = "총알개조 Level 1"; CardDetail = "적 한 명 관통"; }
                break;
            case 2:
                if (barrelCard == 2) { CardName = "총열개조 Level 3"; CardDetail = "탄 퍼짐 60% 감소"; }
                else if (barrelCard == 1) { CardName = "총열개조 Level 2"; CardDetail = "탄 퍼짐 40% 감소"; }
                else { CardName = "총열개조 Level 1"; CardDetail = "탄 퍼짐 20% 감소"; }
                break;
            case 3:
                if (weaknessCard == 2) { CardName = "약점포착 Level 3"; CardDetail = "총알 데미지 50% 증가\n공격속도 20% 증가"; }
                else if (weaknessCard == 1) { CardName = "약점포착 Level 2"; CardDetail = "총알 데미지 40% 증가"; }
                else { CardName = "약점포착 Level 1"; CardDetail = "총알 데미지 20% 증가"; }
                break;
            case 4:
                if (nimblestepsCard == 2) { CardName = "기민한 걸음 Level 3"; CardDetail = "이동속도 +3 증가"; }
                else if (nimblestepsCard == 1) { CardName = "기민한 걸음 Level 2"; CardDetail = "이동속도 +2 증가"; }
                else { CardName = "기민한 걸음 Level 1"; CardDetail = "이동속도 +1 증가"; }
                break;
            case 5:
                if (QuickstepCard == 2) { CardName = "퀵 스텝 Level 3"; CardDetail = "대쉬 쿨타임 3초 감소"; }
                else if (QuickstepCard == 1) { CardName = "퀵 스텝 Level 2"; CardDetail = "대쉬 쿨타임 2초 감소"; }
                else { CardName = "퀵 스텝 Level 1"; CardDetail = "대쉬 쿨타임 1초 감소"; }
                break;
            case 6:
                if (fastdraw == 2) { CardName = "빠른 장전 Level 3"; CardDetail = "즉시 재장전"; }
                else if (fastdraw == 1) { CardName = "빠른 장전 Level 2"; CardDetail = "재장전시 입력해야 할 수 2개 감소"; }
                else { CardName = "빠른 장전 Level 1"; CardDetail = "재장전시 입력해야 할 수 1개 감소"; }
                break;
            default:
                CardName = "";
                CardDetail = "";
